...
* **在更新脚本前，请务必备份您在脚本内 `// --- 用户本地配置区域 ---` 中定义的 `localRules`！** 否则，您的自定义规则可能会在更新时被覆盖。（虽然我大概率以后也不会再更新脚本本体了）
...
4.  根据您想要修改的网站行为，在已启用的 `localRules` 数组中添加或修改规则对象。
    ...
    **示例**：如果您在 Bilibili 视频页面想恢复右键菜单，并且避免弹幕被移除（可能与移除遮罩有关），可以添加或修改针对 Bilibili 的规则：
    ```javascript
    {
      ...
      "unlockOverlay": false      // 如果弹幕问题与常规遮罩层移除相关，可以禁用此项
    }
    ```
...
2.  **GitHub Issues:** 如果您熟悉 GitHub，可以在本仓库的 [Issues 页面]([请在这里替换为您的 GitHub 仓库 Issues 页面链接]) 提交问题（推荐，便于跟踪和交流）。请点击这里提交：👉 **[提交 Issue](https://github.com/MT-Y-TM/Fuck_All_Web_Restrictions/issues/new)**
...
网站 URL: [https://www.some-problem-site.net/article/123](https://www.google.com/url?sa=E&source=gmail&q=https://www.some-problem-site.net/article/123)
问题描述: 文章内容无法复制
...
* 中间尝试过把百度文库撕开，但是失败了，我的技术能力不足以支撑我对抗这种大公司。（有点沮丧）
